...
> 版本 v1.4 · 2026-09-13
> 改动轨迹（每一版都是被一次"说不通"推出来的）：
> **v1.0 → v1.1**：会话是长期实体（原先会话与一次执行绑死，用户第二句话历史就断）。
> **v1.1 → v1.2**：沙箱改成**会话的工作区租约**（按需建 / 热着复用 / 空闲回收），不再一句一个容器。
> **v1.2 → v1.3**：① 空闲 TTL 从**最后一次活动**算起（用到就续时），干活中绝不回收，
> 总寿命到点是**换容器**不是砍任务；② 知识库回到它的真实需求——**用户上传文档 + 每人一份 +
> 手动开关“回答前先召回”**，这是 M3 的事，M2 只留 `knowledge` 分区与开关的接缝（§E.6）。
> **v1.3 → v1.4**：环境构建的**触发点**写死（§C.8）——构建与沙箱/会话**解耦**（它跑在 CP
> 宿主机上，不需要用户说话也不需要容器），所以仓库**第一次被看见**就应后台入队；相应地，
> **Run 自己绝不建环境**：拿不到 `ready` / `degraded` 就用 Layer 1 的语言镜像，并把"依赖还没装"
> 作为沙箱事实写进 system。队列与去重交给 P6/P7（并发上限 1 是 §C.7 已定的）。
...
热着复用的代价是"容器占着内存更久"，所以有两道时间闸，它们的语义必须分清：

> **一句话：30 分钟是"沉默多久就收"（滑动窗口，一有动作就重新数）；6 小时是"一个容器最多活多久"（固定，不续）。**
> 实际感受：你在用 → 一直活着（最长 6h）；你走了 30 分钟 → 被收回；你回来 → 自动建新的（从分支接上），看不出中断。

| 闸 | 计时方式 | 到点那一刻会发生什么 |
|---|---|---|
| **30 分钟的"沉默计时"**（默认，可配） | **滑动窗口**：不是"寿命 30 分钟"，而是"**连续 30 分钟没有任何动作**"。任何动作（你说话 / 模型调用 / 用一次工具 / 跑一条命令）都把倒计时重置为 30 分钟 | ① 此刻真闲着 → 存改动 + 销毁；② 此刻在干活 → **不触发**（干完再重新从零数 30 分钟） |
| **6 小时的"容器寿命"**（不续） | 固定：从容器创建那一刻算 | ① 空闲 → 立刻存改动 + 销毁；② 正在处理你的一句话 → **不打断**，等这句处理完（最多再等 30 分钟：单次执行本身的墙钟上限）→ 存改动 + 销毁；③ 6h30m 后还卡着 → 强杀（卡死兜底） |

**两条绝不违反的规则**：① 正在处理一句话（`active_run_id` 不为空）或沙箱里有在跑的命令时，**绝不回收**；
② 一句话跑了很久（比如 50 分钟）也没关系——期间每次工具调用都在续时，不会跑到一半被停。

**实际顺序通常是 30 分钟那条先触发**（你离开就收）；只有你连续用了 6 小时，才会撞上容器寿命那条。
两条都**不是**"到点就静默下线"：一条只在真闲时生效，另一条会等你手上的活干完。

**换容器会丢什么（必须说清楚）**：只丢"不在 git 里的东西"——比如这次会话里临时装的依赖、构建产物。
代码改动都在任务分支上，新容器从分支恢复。需要时 agent 会重跑一次安装（`npm ci` 之类）。
...
### C.8 谁发起构建：触发点与"Run 绝不建环境"

§C.7 已经定了构建在**CP 宿主机**上跑。这句话还有一个没写出来的推论：**构建跟沙箱、会话、
对话都没有关系**——它不需要用户说话，也不需要容器。由这条性质可以推出两件事：

1. **构建可以、也应该提前做**。首次构建的预算是 p95 < 10 分钟（§J.4）。等用户开口才建，这
   10 分钟只能让用户等，或者让 Run 在没有依赖的环境里跑起来再失败——两种都不应该发生。
   正确做法是**仓库第一次被看见**时就后台走一遍：推断（毫秒级，纯读文件）→ 命中缓存就用
   （§C.5）→ 没命中才构建 → 体检（§C.6）。**缓存键的收益只有"提前建"才拿得到**：
   第二个人、第二次会话秒开的那个体验，就来自这里。
2. **Run 永远不负责建环境**。每次 Run 要用沙箱时只做一次**解析**：有 `ready` / `degraded` 的
   revision 就用它的 digest；没有就用 Layer 1 的语言镜像（§C.2），并把"项目依赖还没装"当成
   **沙箱事实**写进 system（与 §C.6 的 degraded 走同一条通道）。构建失败是合法结果（§C.1），
   它不应该变成"用户的那句话没人处理"。

**触发点只有三个**（每一个都只是"入队"，不改变构建本身的流程）：

| 触发 | 什么时候 | 谁做 |
|---|---|---|
| `first_seen` | 第一次看到某个 `project_key`（会话创建时；M3 的调度器收到 issue 时同理） | CP 编排层：在起 Run 之前**异步入队**，不阻塞 |
| `manual` | 操作者显式要求（P7 的 `--rebuild-env`、`/env/{projectKey}` 页面上的按钮） | 人 |
| `promote` | 会话里验证有效的环境改动固化回项目级（§C.5 的版本化） | 会话结束 / 用户明确要求 |

**为什么每次尝试要记触发原因**：`env_builds` 一行是一次尝试，只记"什么时候建的"回答不了
"这些构建为什么发生"（是没人用的仓库被批量预热了？还是有人手工点了三次？）。所以每行带一个
`trigger`。**队列与去重**（同一个 `project_key` 已有构建在跑、或已经是 `ready` 时不重复入队）
同样是 P6/P7 的交付物：构建并发上限是 1（§C.7），没有队列的话，多个仓库同时进来就只是
"排在一个信号量后面"，行为不可见、失败不可解释。

**M3 会多一个入口，但不多一层机制**：调度器收到 issue 时它已经知道是哪个仓库了，于是它可以在
起 Run 之前就把环境排队——那仍然是 `first_seen` 这一次触发，只是发起人从会话变成了调度器。
真要做"仓库一接进来就建"（仓库登记表 / 连接仓库的界面）是另一件事，它增加的是**触发时机**，
不是构建机制；那时给 `trigger` 加一个取值即可（迁移只增不改）。
...
| 知识库召回（用户上传文档） | pgvector（M3） | ❌ **M2 不做** | 它是"用户文档知识库"，不是"代码语义搜索"；属于 M3（见 §H） |
...
| `seed` | 检索到的"该看哪几个文件"（只有路径 + 符号名，不含正文） | ≤ 3% | 一次执行内不变 | 跟随 task |
| `history` | 近期 entries（完整） | 剩余全部 | 每轮增长 | 不缓存（追加式） |
| `knowledge` | **预留**：用户自己的文档知识库召回（M3，见 §E.6） | ≤ 10%（开启时） | 每轮可变 | 不缓存 |

`knowledge` 这一行现在**不实现**：没有配置知识库时它根本不存在（编译产物与 M2 完全一致）。
预留它只是为了将来加一个分区，而不是改编译器。
...
需求已经明确（注意：这**不是**“代码语义搜索”，那是另一个东西）：**用户上传自己的文档，
每人/每项目一份知识库，并且可以手动开关“回答之前先召回”**。M2 不做，但接缝现在就留好，
以后加它不用改编译器：

1. **分区**：`knowledge`（§E.2 已留行）——开启召回时，这里是 top-k 片段 + 来源引用；
2. **开关**：会话级默认值 + 每条消息可覆盖（`autoRecall: on|off`）。**默认关**：用户没上传文档
   或没打开开关时，编译产物与 M2 完全一致（这也是“留接缝 ≠ 提前实现”的可验证形式）；
3. **关掉召回也能用**：给模型一个 `search_knowledge` 工具（与自动召回互补——自动召回解决
   “它不知道我上传了啥”，工具解决“它需要多翻几页”）；
4. **引用是硬要求**：召回进上下文的每一段都必须带来源（文件名 + 片段位置），答案里才能要求它
   标注出处——没有引用的知识库输出无法验证，等于把幻觉换了个来源；
5. **信任边界**：上传的文档是**不可信内容**（与仓库内容同级）——可以被引用，不能改变系统策略、
   不能绕过审批；多用户上线时，知识库的隔离与沙箱隔离是同一道门槛（gVisor + 每用户独立网络）。

**为什么不放在 M2**：M2 的主题是“任意仓库能跑起来 + agent 懂仓库”。知识库与仓库无关
（用户可能对一个空仓库也要问自己上传的手册），把两件事绑在一起只会让 M2 的验收口径变模糊。
触发条件写在 §H。
...
| **知识库（用户上传文档 + 向量召回）** | 它是独立的产品面（上传 / 切片 / 索引 / **召回开关** / 引用来源），与"仓库索引"无关；M2 只需要把分区与开关的接缝留好（§E.2 / §E.6） | M3：每用户 / 每项目一个知识库，可手动开关"回答前先召回" |
...
ContextCompiler 的分区        │ 知识库召回（用户上传文档）作为新分区 + 召回开关 │ 分区是可插拔的；`knowledge` 名字已预留
...
1. **环境**：`agent:run --repo <url>` 不需要任何手工准备 → 10 分钟内得到环境状态
   ...
   （这一条说的是**环境管线自己的**口径：构建由 `first_seen` 异步入队，**Run 不等它**——
   没有现成环境时 Run 用 Layer 1 的镜像跑，构建结果在环境页/日志里看。要看着它跑完，
   用 `--rebuild-env` 或等队列空——见 §C.8。两个行为都要有，因为"等 10 分钟"与"立刻能用"
   不能兼得，而"用户的话没人处理"是不能接受的）
...
| P6 LLM 生成 + 自愈（含构建队列：并发 1 / 去重 / `trigger`） | 3–4 | 中（成本失控 / 死循环） | ✅ |
