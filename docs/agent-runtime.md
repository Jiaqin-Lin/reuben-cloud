...
> **一句话：一个容器最多活 6 小时；但只要它闲着超过 30 分钟，就提前收掉。**
> 实际感受：你在用 → 一直活着（最长 6h）；你走了 30 分钟 → 被收回；你回来 → 自动建新的（从分支接上），看不出中断。

| 闸 | 从什么时候算 | 到点了干什么 |
|---|---|---|
| **空闲 TTL**（默认 30 分钟，可配） | **从最后一次活动算起**：用户发言、每次模型调用、每次工具调用（含沙箱文件 API）都算活动，都会续时 | 真闲着才回收（落地改动 + 销毁） |
| **容器总寿命**（6h，不续） | 从建容器算起 | **不砍任务**：等当前活动结束（包括在跑的 exec 跑到它自己的超时）→ 落地 → 换一个新容器；用户只会看到"下一句慢一点" |

**两条绝不违反的规则**：① 正在处理一句话（`active_run_id` 不为空）或沙箱里有在跑的命令时，**绝不回收**；
② 一句话跑了很久（比如 50 分钟）也没关系——期间每次工具调用都在续时，不会跑到一半被停。
唯一能强制回收的是"真卡死"：超过总寿命再加 30 分钟宽限仍无任何活动结束（这是给`跑飞的进程`准备的，不是常规路径）。
