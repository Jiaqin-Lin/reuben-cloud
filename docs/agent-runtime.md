...
└───────────┬─────────────────────────────────────────────────────────────┘
            │ 9 个端点（不变）
┌───────────▼─────────────────────────────────────────────────────────────┐
│ Sandbox（每 Run 一个）  image = Environment 的产物 digest                 │
...
（M4 的 Eval Harness）再上——这与 README §3.8 里"没有评估集的进化是随机游走"是同一条原则。
...
**四个部分，14 个 Phase**，按依赖排序。详细实现步骤在 spec 里；这里只给"为什么是这个顺序"与估算。

```
第一部分 · Agent Runtime（先做，因为它决定后面所有东西的形状）
  P1 运行时契约与包拆分 ──► P2 会话持久化 ──► P3 compaction ──► P4 事件统一
      （3-4d）                 （4-5d）           （3-4d）         （1-2d）
                                        │
...
第三部分 · 索引与上下文（依赖 P1-P4 的存储与循环，也依赖 P5-P7 的"能跑起来")  │
  P8 符号索引 ──────► P9 Repo Map ──────► P10 ContextCompiler ──► P11 向量（触发式）
      （4-6d）           （3-4d）             （4-5d）                （3d）
                                        │
第四部分 · 工具面（依赖 P1 的契约；可与第三部分交错）                        │
  P12 Registry + 7 工具 ──► P13 Skills ──► P14 MCP                        ◄──┘
      （3-4d）                （2-3d）        （3-5d）
```
...
| README §5 · M2 | "仓库索引（tree-sitter + ripgrep + pgvector）" | ripgrep 是**工具**不是索引；pgvector 改为**触发式**（不在 M2 默认范围） | 分类错误 + 没有评估集的检索不上线 |
