...
- 打开页面能看到一次 Run 的实时流（手工；`npm run agent:run -- --serve` 之后打开它打印的地址）
- SSE 端点断开后浏览器自动重连能接上（`EventSource` 自带 `Last-Event-ID`）

#### 自动化到哪一步（比正文多，理由见备注 12）

| 要点 | 自动化形式 |
|---|---|
| 实时流里三类事件都在 | `test/integration/web-stream.integration.test.ts`：真沙箱 + 真 HTTP，断言 `run_start → turn → tool_call → exec_start → exec_output → exec_end → tool_result → turn → run_end` 的完整序列 |
| 重连不重不漏 | 单测 `web-server.test.ts` 用 `Last-Event-ID` 请求补发；集成测试在真流上从第 3 条断点接回去，断言下一条正好是第 4 条 |
| 事件词汇的映射 | 单测 `agent-events.test.ts`：沙箱事件（含坏 JSON / 认不出的事件名）→ `RunEvent`，以及循环/工具的埋点（含"观察者抛异常不弄死 Run"） |
| 缓冲与重放的边界 | 单测 `web-hub.test.ts`：游标落在淘汰区时先给 `gap`、文本增量合并、两个上限、订阅者积压、run 淘汰 |
| 前端资源不出错 | `packages/web/test/ui.test.ts`：`index.html` 引的东西都在静态白名单里、`app.js` 语法可解析且每个事件类型都有分支、不引外部资源、不拼 HTML、对比度到 AA |

浏览器那一半（`EventSource` 会自动带 `Last-Event-ID`）是规范行为，没有实现可测；
所以我们把可断言的那一半（服务端按游标补发）真的断言了，而不是写一句"手工验过"。

### 验收标准

- [x] 一次 Run 的全过程能在浏览器里实时看到（手工：`--serve` 之后打开页面；另有一条真容器 + 真 HTTP 的集成测试把这条链路自动跑一遍）
- [x] 三类事件（模型文字增量 / 工具调用与结果 / 沙箱命令输出）在同一条流上
- [x] 断线重连按 `Last-Event-ID` 接上（不重不漏）
- [x] 只读：页面上没有一个写操作的入口（没有鉴权、只监听 127.0.0.1）
...
1. **多了一个 `node:http` 服务（`src/web/server.ts`），而且它是 CP 的第一个 HTTP 服务**。
   正文说"CP 增加一个 SSE 端点"时，CP 上还没有任何 HTTP 服务——所以这一步必然是"起一个
   服务器 + 五条只读路由"。**不用 Hono/Express 的理由**：M0 只需要五条路由、零依赖的
   `node:http` 加一个 switch 就够；README 里画的 Hono 是 M1（那时才有真正的 API 面）。
2. **多了一个 `src/web/hub.ts`（正文没提）**。SSE 端点不能直接挂在循环上：`Last-Event-ID`
   的重放需要**一个有界的事件缓冲**，而不是"发出去就没了"。三件事只有它能做对：
   ① 环形缓冲（默认 2000 条 / 8 MiB，谁先到算谁）+ 淘汰区游标的 `gap` 说明；
   ② 多个订阅者（刷新页面会短暂出现两条连接）；③ 循环 `emit()` 不等任何消费者。
   先例是 Phase 1 的沙箱事件总线（同一个问题、同一套解法）。
3. **文本增量按 80ms 合并**。`onText` 的粒度是 token：一次 40 轮的 Run 能产生上万条增量，
   逐条进缓冲会把工具结果挤掉、让浏览器为一两个字符各跑一次布局。合并规则只有一条：
   **攒到 80ms 或下一条非文本事件到达时一起发**（顺序不能乱：文本属于它前面那一轮）。
4. **事件词汇是新的一份（`src/agent/events.ts`），不是复用 transcript 的记录**。两者在同一批
   位置发射，但受众不同：transcript 是**事后取证**（每轮完整 system + tools + messages，几十 MB
   也要写），事件流是**实时观察**（一条一个 JSON，前端一个 switch 就能渲染）。把 transcript 的
   记录推给浏览器等于每轮重推几百 KB；把事件流当证据存又会丢掉完整上下文。
5. **文字只有一条通道**：循环**不**自己发 `text` 事件，调用方把 `onText` 接到 sink 上
   （`scripts/agent-run.ts` 就是这么接的：同时进 stdout 与事件流）。否则同一个增量会被发两遍。
6. **沙箱事件 → RunEvent 是一层独立的映射（`createExecEventMapper`）**。前端不该认识第二套协议
   （snake_case 字段、`completed` 这种事件名）。两个细节：映射器记得 `executionId`（沙箱的
   stdout/stderr 事件**不带**它，而前端需要它把输出归到某条命令上，靠的是 `started` 事件先到）；
   认不出的/解不开的事件变成一条 `note` 而不是异常（协议漂了要能看见，但不能弄死一次 Run）。
7. **观察者是旁路：`emitEvent()` 吞异常只记一条 warn**。一次已经改了 20 分钟代码的 Run 不该
   因为一个渲染 bug 归零（与 `sandbox-manager` 的 `onEvent` 同一条规矩），有一条单测专门钉它。
8. **静态资源是白名单查表，不是把目录挂上去**（`src/web/static.ts`）。URL 路径只当 Map 的键用，
   **不做任何路径拼接**，于是路径穿越（`..`、URL 编码、符号链接）这整类问题不存在。
   只有三个文件；`/favicon.ico` 直接回 204（不引二进制资源，也消掉控制台那条 404）。
9. **没有鉴权 ⇒ 只监听 127.0.0.1，而且不提供 `--host` 开关**。spec 把鉴权划到 M3，那么"谁能
   访问"就只能靠绑定地址回答：一个无鉴权的 HTTP 服务监听 `0.0.0.0` 是网络里最便宜的一个洞。
   想远程看就 SSH 隧道。
10. **多了 `GET /runs/{id}/info`（正文没有）**。`EventSource` 看不到 HTTP 状态码（404 与网络
    抖动在它眼里都是 `onerror`），页面必须能区分"这个 run 不存在"（别重试了）与"抖了一下"
    （浏览器自己重试）。这个响应还带 model / issue / totalEvents，页面用它显示"Run 已经在跑，
    等第一个事件"这个状态。
11. **`--serve` 跑完不退出，Ctrl-C 才退**。缓冲只在内存里，进程一退页面就只剩下"找不到这个
    Run"。不 `--serve` 时脚本行为与以前完全一样（不监听任何端口）。
12. **前端的取舍：无构建、无外部资源、无图标库、系统字体栈**。三条外部依赖（字体 CDN、图标
    库、分析脚本）在本机回环的开发工具上只有代价、没有收益。`app.js` 里**一个 import 都没有**，
    所以 `test/ui.test.ts` 能用 `vm.Script` 把整份文件做一次语法检查——一个笔误不该等到
    "打开浏览器发现白屏"。视觉上是对 **Anthropic / Claude 的语言的近似**（暖中性底色 +
    单一陶土色强调、消息没有气泡、等宽只给"机器说的话"），**不是官方资源**；浅/深色对比度
    （正文 15.8:1 / 次要 5.3:1 / 实心按钮 4.9:1）由测试算，不是目测。
13. **工具结果与命令输出在页面上都是有界的**：命令输出只保留最后 400 行（更早的折成一行
    说明），工具结果默认折叠且只渲染前 300 行。不设这两条的话，跑一个 `npm install` 就能让观察窗
    自己卡死——而卡死的正是"能看清每一步"这个前提。
14. **`verifyInSandbox`（Phase 12 的验证命令）暂时不进事件流**。它也是沙箱 exec，但它的位置在
    循环**之后**、`finishRun()` 里，而 Phase 13 的观察窗只管一次 Run 的 agent 循环。
    要接的话就是给 `verifyInSandbox` 也传一个 sink——那属于 M3 的"完整 Trace"。
