...
| 沙箱总寿命（安全兜底） | 6h | **不砍任务**：等当前活动结束 → archive + destroy（下次用重建） |
| **会话空闲 TTL**（M2 起，回收的主机制） | 30min，**从最后一次活动算起** | 真闲着才回收：先落地改动（取 diff → push），再 destroy |

总寿命 TTL 是**安全兜底**，不是资源调度策略：一个跑飞的 agent 不能无限占着容器。
但它与单条命令的超时是两层：**命令由它自己的 120s/600s 超时收掉**，容器总寿命到点时已经不会有
长时间在跑的东西了，所以正常路径下不需要强杀；只有"超过 6h30m 仍无任何活动结束"才强制回收。

> **M2 起多了一层"会话空闲 TTL"**：沙箱从"一次 Run 一个、用完即销"变成"**会话的工作区租约**"——
> 按需创建、热着复用（同一会话的下一句不重建）、闲置才回收（回收前必须把改动落到任务分支）。
>
> **这个计时器必须会续时，而且绝不能打断在干的活**：用户发言、每次模型调用、每次工具调用
> （含文件 API）都把截止时间往后推；正在处理一句话（`active_run_id` 不为空）或沙箱里有在跑的
> exec 时，到点也不回收。一句话跑 50 分钟不会被砍。
>
> 上面那张表里的其余超时/状态机/加固参数**一字不改**；变的只是"谁来申请、什么时候申请、什么时候释放"。
