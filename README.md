...
**这是另一件事，不要和"代码语义检索"混在一起**：用户上传自己的文档（手册 / 规范 / 需求文档），
每人 / 每项目一份知识库，并且**可以手动开关“回答之前先召回”**。它需要的向量库（pgvector）是为这件事准备的，
不是为了搜代码。

M2 只留接缝，不实现：`ContextCompiler` 的 `knowledge` 分区（预留）、召回开关的语义（**默认关**，
关掉时编译产物与 M2 完全一致）、以及一个 `search_knowledge` 工具作为关掉自动召回时的补充。
设计要点（引用来源、文档是不可信内容、多用户隔离与沙箱同一道门槛）见
[`docs/agent-runtime.md`](docs/agent-runtime.md) §E.6。
...
| 数据库 | Postgres | 状态 + 审计 + 会话/用量一把梭，少一个组件（知识库召回 M3 时再加 pgvector 扩展） |
...
- [ ] **知识库**（用户上传文档 + 每用户/每项目一份 + 手动开关“回答前先召回” + 引用来源；pgvector 在这一步才引入）
- [ ] 密钥代理
- [ ] 完整 Trace + 回放 + 成本看板
- [ ] 验证阶梯 + 可信度评分
- [ ] Project Memory
...
| **成本失控** | 沙箱 + token 持续漏钱 | 会话级空闲 TTL（默认 30min，从最后一次活动算，用到就续时）+ 容器总寿命 6h 硬上限；回收前先落地；预热池复用 / 快照挂起（M3）；配额；成本归因到 Run；模型分级路由 |
| **会话热着复用沙箱** | 空闲容器占内存；硬崩会丢"上次落地之后"的改动 | 空闲 TTL 从**最后一次活动**算起（用户发言 / 模型调用 / 工具调用都续时）；正在干活或在跑命令时绝不回收；容器总寿命 6h 到点是**换容器**不是砍任务；回收前先落地（推送失败就不销毁，最多重试 3 次后落 archive）；`ERROR(container_lost)` 结构化上报丢的时段 |
